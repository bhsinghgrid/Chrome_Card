function getUiMountNode() {
  // Some sites apply CSS transforms to <body>, which can break `position: fixed` when our
  // UI is mounted under <body>. In that case, prefer mounting under <html>.
  if (!document.documentElement) return document.body;
    ...
    if (sidebarOpen) {
      ...
      if (sidebar) {
        ...
        setTimeout(() => {
          ...
          if (window.__googleDashboardSidebarResizeHandler) {
            window.removeEventListener("resize", window.__googleDashboardSidebarResizeHandler);
            delete window.__googleDashboardSidebarResizeHandler;
          }
...
window.refreshData = function(e) {
  const evt = e || (typeof event !== 'undefined' ? event : null);
...
function setAuthStatusPill(el, kind, text) {
  if (!el) return;
  el.textContent = text;
...
window.checkAuthStatus = function() {
  ...
  if (!pill) return;
  setAuthStatusPill(pill, 'checking', 'Checking…');
  showAuthError('');

  chrome.runtime.sendMessage({ type: 'CHECK_AUTH' }, (res) => {
    if (chrome.runtime.lastError) {
      setAuthStatusPill(pill, 'disconnected', 'Not connected');
      showAuthError(chrome.runtime.lastError.message);
      return;
    }
    if (!res || !res.success) {
      setAuthStatusPill(pill, 'disconnected', 'Not connected');
      showAuthError((res && res.error) || 'No response from background');
      return;
    }
    if (res.connected) {
      setAuthStatusPill(pill, 'connected', '✓ Connected');
      showAuthError('');
    } else {
      setAuthStatusPill(pill, 'disconnected', 'Not connected');
...
window.login = function() {
  const pill = document.getElementById('sync-status-pill');
  if (pill) setAuthStatusPill(pill, 'checking', 'Logging in…');
  showAuthError('');

  chrome.runtime.sendMessage({ type: 'LOGIN' }, (res) => {
    if (chrome.runtime.lastError) {
      if (pill) setAuthStatusPill(pill, 'disconnected', 'Not connected');
      showAuthError(chrome.runtime.lastError.message);
      return;
    }
    if (!res || !res.success) {
      if (pill) setAuthStatusPill(pill, 'disconnected', 'Not connected');
      showAuthError((res && res.error) || 'Login failed');
      return;
    }
...
window.logout = function() {
  const pill = document.getElementById('sync-status-pill');
  if (pill) setAuthStatusPill(pill, 'checking', 'Logging out…');
  showAuthError('');
  ...
  chrome.runtime.sendMessage({ type: 'LOGOUT' }, (res) => {
    ...
    if (!res || !res.success) {
      if (pill) setAuthStatusPill(pill, 'disconnected', 'Not connected');
      showAuthError((res && res.error) || 'Logout failed');
      return;
    }
...
function initializeExtension() {
  ...
  try {
    ...
    const attachMountObserver = () => {
      ...
      mountObserver.observe(mount, { childList: true, subtree: false });
    };

    // Watch for button removal from the current mount.
    attachMountObserver();

    // Watch for <body> replacement (some SPAs), then re-attach mount observer.
  ...
  function createSidebar(button) {
    ...
    function applyResponsiveSidebarSizing() {
      const screenWidth = window.innerWidth;
      ...
      // Push content to the left (keeps page readable while sidebar is open).
      document.body.style.marginRight = sidebarWidth;
      document.documentElement.style.marginRight = sidebarWidth;
    }

    applyResponsiveSidebarSizing();
    
    // Update sizing live on resize while sidebar is open.
    const resizeHandler = () => {
      const activeSidebar = document.getElementById("google-sidebar");
      if (!activeSidebar) return;
      applyResponsiveSidebarSizing();
    };
    window.__googleDashboardSidebarResizeHandler = resizeHandler;
    ...
    sidebar.innerHTML = `
      ...
      <div id="settings-tab" class="tab-content">
        <div class="section-container" style="padding: 20px;">
          ...
          <div style="margin-bottom: 8px; margin-top: 20px; color: #60a5fa; font-weight: 600; font-size: 13px;">🧩 MCP BACKEND</div>
          
          <div class="settings-group" style="background: rgba(255,255,255,0.03); padding: 14px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.08); margin-bottom: 20px; padding-bottom: 14px; border-bottom: none;">
            <div style="margin-bottom: 10px; color: #cbd5e1; font-size: 11px; line-height: 1.5;">
              Set your local Java backend URL (Spring Boot) that proxies your MCP servers.
            </div>
            ...
            <div style="display: flex; gap: 8px; align-items: center;">
              <input 
                ...
                style="flex: 1; padding: 10px 12px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.15); color: white; border-radius: 8px; font-size: 12px; box-sizing: border-box;"
              >
              <button 
                id="mcp-backend-save-btn"
                style="padding: 10px 12px; background: rgba(96, 165, 250, 0.15); border: 1px solid rgba(96, 165, 250, 0.4); color: #60a5fa; border-radius: 8px; cursor: pointer; font-size: 12px; font-weight: 600; white-space: nowrap; transition: all 0.3s ease;"
            ...
            <div id="mcp-backend-error" style="display: none; margin-top: 10px; color: #EF5350; font-size: 12px; padding: 10px; background: rgba(239, 83, 80, 0.15); border-radius: 6px; border-left: 3px solid #EF5350;"></div>
            
            <div style="margin-top: 12px; display: grid; grid-template-columns: 1fr; gap: 10px;">
              <div style="display: flex; justify-content: space-between; align-items: center; gap: 8px;">
                <div style="min-width: 0;">
                  <div style="font-weight: 600; font-size: 12px; color: #e2e8f0;">Calendar MCP</div>
                  <div id="mcp1-url" style="font-size: 11px; color: #94a3b8; word-break: break-all;"></div>
                </div>
                <div style="display: flex; align-items: center; gap: 8px;">
                  <span id="mcp1-status" style="padding: 4px 8px; border-radius: 999px; font-size: 11px; font-weight: 700; border: 1px solid rgba(255,255,255,0.15); background: rgba(255,255,255,0.06); color: #cbd5e1;">Unknown</span>
                  <button id="mcp1-check-btn" style="padding: 6px 10px; background: rgba(52, 168, 83, 0.12); border: 1px solid rgba(52, 168, 83, 0.35); color: #4ade80; border-radius: 8px; cursor: pointer; font-size: 11px; font-weight: 700; transition: all 0.3s ease;">
                    Check
                  </button>
                </div>
              </div>
              ...
              <div style="display: flex; justify-content: space-between; align-items: center; gap: 8px;">
                <div style="min-width: 0;">
                  <div style="font-weight: 600; font-size: 12px; color: #e2e8f0;">Email MCP</div>
                  <div id="mcp2-url" style="font-size: 11px; color: #94a3b8; word-break: break-all;"></div>
                </div>
                <div style="display: flex; align-items: center; gap: 8px;">
                  <span id="mcp2-status" style="padding: 4px 8px; border-radius: 999px; font-size: 11px; font-weight: 700; border: 1px solid rgba(255,255,255,0.15); background: rgba(255,255,255,0.06); color: #cbd5e1;">Unknown</span>
                  <button id="mcp2-check-btn" style="padding: 6px 10px; background: rgba(52, 168, 83, 0.12); border: 1px solid rgba(52, 168, 83, 0.35); color: #4ade80; border-radius: 8px; cursor: pointer; font-size: 11px; font-weight: 700; transition: all 0.3s ease;">
                    Check
                  </button>
                </div>
              </div>
          ...
          <div style="margin-bottom: 8px; margin-top: 20px; color: #60a5fa; font-weight: 600; font-size: 13px;">🤖 MCP ASSISTANT</div>
          
          <div class="settings-group" style="background: rgba(255,255,255,0.03); padding: 14px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.08); margin-bottom: 20px;">
            <div style="margin-bottom: 10px; color: #cbd5e1; font-size: 11px; line-height: 1.5;">
              Write a prompt and run it through the backend agent at <span style="font-family: monospace;">/api/agent/chat</span>.
              <div style="margin-top: 6px; opacity: 0.9;">
                Example: <span style="font-family: monospace;">List my next 5 events</span> or <span style="font-family: monospace;">Schedule a meeting with ...</span>
              </div>
            </div>
            ...
            <textarea
              id="mcp-agent-prompt"
              rows="4"
              placeholder='Try: "List my next 5 calendar events"'
              style="width: 100%; padding: 10px 12px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.15); color: white; border-radius: 8px; font-size: 12px; box-sizing: border-box; resize: vertical;"
            ></textarea>
            
            <div style="display: flex; gap: 8px; align-items: center; margin-top: 10px;">
              <button
                id="mcp-agent-send-btn"
                style="flex: 1; padding: 10px 12px; background: rgba(96, 165, 250, 0.15); border: 1px solid rgba(96, 165, 250, 0.4); color: #60a5fa; border-radius: 8px; cursor: pointer; font-size: 12px; font-weight: 700; transition: all 0.3s ease;"
                onmouseover="this.style.background='rgba(96, 165, 250, 0.25)'"
                onmouseout="this.style.background='rgba(96, 165, 250, 0.15)'"
              >
                Send Prompt
              ...
              <button
                id="mcp-agent-clear-btn"
                style="padding: 10px 12px; background: rgba(239, 83, 80, 0.12); border: 1px solid rgba(239, 83, 80, 0.35); color: #fca5a5; border-radius: 8px; cursor: pointer; font-size: 12px; font-weight: 700; transition: all 0.3s ease;"
              >
                Clear
              </button>
            </div>
            
            <div id="mcp-agent-error" style="display: none; margin-top: 10px; color: #EF5350; font-size: 12px; padding: 10px; background: rgba(239, 83, 80, 0.15); border-radius: 6px; border-left: 3px solid #EF5350;"></div>
            
            <div id="mcp-agent-response" style="display: none; margin-top: 10px; color: #e2e8f0; font-size: 12px; padding: 10px; background: rgba(52, 168, 83, 0.08); border-radius: 6px; border-left: 3px solid rgba(52, 168, 83, 0.6); white-space: pre-wrap;"></div>
            
            <details id="mcp-agent-steps" style="display: none; margin-top: 10px;">
              <summary style="cursor: pointer; color: #cbd5e1; font-size: 11px;">Show tool steps</summary>
              <pre id="mcp-agent-steps-pre" style="margin-top: 8px; padding: 10px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.10); color: #cbd5e1; border-radius: 8px; font-size: 11px; white-space: pre-wrap; overflow: auto;"></pre>
            </details>
          </div>
          ...
          <div class="settings-group">
            <div class="settings-label">
              ...
              <span id="sync-status-pill" style="padding: 4px 8px; border-radius: 999px; font-size: 11px; font-weight: 800; border: 1px solid rgba(255,255,255,0.15); background: rgba(255,255,255,0.06); color: #cbd5e1;">Checking…</span>
            </div>
            <div id="auth-error" style="display: none; margin-top: 10px; color: #EF5350; font-size: 12px; padding: 10px; background: rgba(239, 83, 80, 0.15); border-radius: 6px; border-left: 3px solid #EF5350;"></div>
          </div>
          ...
          <div class="btn-group">
            <button id="settings-refresh-btn" class="settings-btn">🔄 Refresh Now</button>
            <button id="settings-clear-cache-btn" class="settings-btn">🗑️ Clear Cache</button>
          </div>
          
          <div class="btn-group">
            <button id="settings-login-btn" class="settings-btn" style="flex: 1;">🔐 Login</button>
            <button id="settings-logout-btn" class="settings-btn" style="flex: 1;">🚪 Logout</button>
          </div>
    ...
    const mount = getUiMountNode();
    if (mount) mount.appendChild(sidebar);
    ...
    setTimeout(() => {
      ...
      // Bind settings buttons by id (avoid index-based binding)
      const refreshBtn = document.getElementById('settings-refresh-btn');
      if (refreshBtn) refreshBtn.onclick = (e) => window.refreshData(e);

      const clearCacheBtn = document.getElementById('settings-clear-cache-btn');
      if (clearCacheBtn) clearCacheBtn.onclick = () => window.clearCache();
      ...
      const backendUrlInput = document.getElementById('mcp-backend-url');
      const backendSaveBtn = document.getElementById('mcp-backend-save-btn');
      const backendErrorDiv = document.getElementById('mcp-backend-error');
      const mcp1UrlDiv = document.getElementById('mcp1-url');
      const mcp2UrlDiv = document.getElementById('mcp2-url');
      const mcp1Status = document.getElementById('mcp1-status');
      const mcp2Status = document.getElementById('mcp2-status');
      const mcp1CheckBtn = document.getElementById('mcp1-check-btn');
      const mcp2CheckBtn = document.getElementById('mcp2-check-btn');

      const MCP_DEFAULT_BACKEND_URL = 'http://127.0.0.1:8082';

      function showBackendError(message) {
        if (!backendErrorDiv) return;
        if (!message) {
          backendErrorDiv.style.display = 'none';
          backendErrorDiv.textContent = '';
          return;
        }
        backendErrorDiv.textContent = `❌ ${message}`;
      ...
      function updateMcpUrls(baseUrl) {
        if (mcp1UrlDiv) mcp1UrlDiv.textContent = baseUrl ? `${baseUrl}/mcp/sse` : '';
        if (mcp2UrlDiv) mcp2UrlDiv.textContent = baseUrl ? `${baseUrl}/mcp2/sse` : '';
      }

      function checkMcpHealth(which, baseUrl) {
        const statusEl = which === 'mcp2' ? mcp2Status : mcp1Status;
        setStatusPill(statusEl, 'checking', 'Checking…');
        chrome.runtime.sendMessage({ type: 'CHECK_MCP_HEALTH', which, baseUrl }, (res) => {
          if (chrome.runtime.lastError) {
            const label = which === 'mcp2' ? 'Email MCP' : 'Calendar MCP';
            showBackendError(`${label} check failed: ${chrome.runtime.lastError.message}`);
            setStatusPill(statusEl, 'down', 'Down');
            return;
          }
          if (!res) {
            const label = which === 'mcp2' ? 'Email MCP' : 'Calendar MCP';
            showBackendError(`${label} check failed: no response from background`);
            setStatusPill(statusEl, 'down', 'No response');
            return;
          }
          if (!res.success) {
            const label = which === 'mcp2' ? 'Email MCP' : 'Calendar MCP';
            showBackendError(`${label} check failed: ${res.error || 'unknown error'}`);
            setStatusPill(statusEl, 'down', 'Down');
            return;
          }
          ...
          if (status === 'ok') {
            showBackendError('');
            setStatusPill(statusEl, 'ok', 'OK');
          } else {
            const label = which === 'mcp2' ? 'Email MCP' : 'Calendar MCP';
            showBackendError(`${label} is not healthy (status: ${status})`);
            setStatusPill(statusEl, 'down', 'Down');
          }
        });
      }

      // Init: load saved backend URL + render derived URLs.
      chrome.storage.local.get({ mcpBackendUrl: MCP_DEFAULT_BACKEND_URL }, (store) => {
        const baseUrl = store.mcpBackendUrl || MCP_DEFAULT_BACKEND_URL;
        if (backendUrlInput) backendUrlInput.value = baseUrl;
        updateMcpUrls(baseUrl);
        setStatusPill(mcp1Status, 'unknown', 'Unknown');
        setStatusPill(mcp2Status, 'unknown', 'Unknown');
      });

      if (backendSaveBtn && backendUrlInput) {
        backendSaveBtn.onclick = () => {
          showBackendError('');
          let normalized;
          try {
            normalized = normalizeBackendUrl(backendUrlInput.value);
          } catch (e) {
            showBackendError('Invalid backend URL. Example: http://localhost:8080');
            return;
          }

          chrome.storage.local.set({ mcpBackendUrl: normalized }, () => {
            updateMcpUrls(normalized);
            setStatusPill(mcp1Status, 'unknown', 'Unknown');
            setStatusPill(mcp2Status, 'unknown', 'Unknown');
          });
        };
      ...
      if (mcp1CheckBtn && backendUrlInput) {
        mcp1CheckBtn.onclick = () => {
          showBackendError('');
          let normalized;
          try {
            normalized = normalizeBackendUrl(backendUrlInput.value || MCP_DEFAULT_BACKEND_URL);
          } catch {
            showBackendError('Invalid backend URL. Example: http://localhost:8080');
            return;
          }
          checkMcpHealth('mcp', normalized);
        };
      }

      if (mcp2CheckBtn && backendUrlInput) {
        mcp2CheckBtn.onclick = () => {
          showBackendError('');
          let normalized;
          try {
            normalized = normalizeBackendUrl(backendUrlInput.value || MCP_DEFAULT_BACKEND_URL);
          ...
          checkMcpHealth('mcp2', normalized);
        };
      }

      // ─── MCP Assistant prompt ────────────────────────────────────────────
      const agentPromptInput = document.getElementById('mcp-agent-prompt');
      const agentSendBtn = document.getElementById('mcp-agent-send-btn');
      const agentClearBtn = document.getElementById('mcp-agent-clear-btn');
      const agentErrorDiv = document.getElementById('mcp-agent-error');
      const agentResponseDiv = document.getElementById('mcp-agent-response');
      const agentStepsDetails = document.getElementById('mcp-agent-steps');
      const agentStepsPre = document.getElementById('mcp-agent-steps-pre');

      function showAgentError(message) {
        if (!agentErrorDiv) return;
        if (!message) {
          agentErrorDiv.style.display = 'none';
          agentErrorDiv.textContent = '';
          return;
        }
        agentErrorDiv.textContent = `❌ ${message}`;
        agentErrorDiv.style.display = 'block';
      }

      function showAgentResponse(text) {
        if (!agentResponseDiv) return;
        if (!text) {
          agentResponseDiv.style.display = 'none';
          agentResponseDiv.textContent = '';
          return;
        }
        agentResponseDiv.textContent = text;
        agentResponseDiv.style.display = 'block';
      }

      function showAgentSteps(steps) {
        if (!agentStepsDetails || !agentStepsPre) return;
        if (!Array.isArray(steps) || steps.length === 0) {
          agentStepsDetails.style.display = 'none';
          agentStepsPre.textContent = '';
          return;
        }
        agentStepsPre.textContent = JSON.stringify(steps, null, 2);
        agentStepsDetails.style.display = 'block';
      }

      function getNormalizedBackendUrlOrDefault() {
        const raw = backendUrlInput ? backendUrlInput.value : MCP_DEFAULT_BACKEND_URL;
        try {
          return normalizeBackendUrl(raw || MCP_DEFAULT_BACKEND_URL);
        } catch {
          return MCP_DEFAULT_BACKEND_URL;
        }
      }

      function setAgentSending(isSending) {
        if (!agentSendBtn) return;
        agentSendBtn.disabled = Boolean(isSending);
        agentSendBtn.textContent = isSending ? '⏳ Running…' : 'Send Prompt';
      }

      function sendAgentPrompt() {
        if (!agentPromptInput) return;
        const prompt = (agentPromptInput.value || '').trim();
        if (!prompt) {
          showAgentError('Please enter a prompt.');
          return;
        }

        showAgentError('');
        showAgentResponse('Running…');
        showAgentSteps([]);
        setAgentSending(true);

        const baseUrl = getNormalizedBackendUrlOrDefault();
        chrome.storage.local.get({ mcpAgentSessionId: 'session_default' }, (store) => {
          const sessionId = store.mcpAgentSessionId || 'session_default';
          chrome.runtime.sendMessage(
            { type: 'MCP_AGENT_CHAT', baseUrl, message: prompt, sessionId },
            (res) => {
              setAgentSending(false);

              if (chrome.runtime.lastError) {
                showAgentResponse('');
                showAgentError(chrome.runtime.lastError.message);
                return;
              }
              if (!res) {
                showAgentResponse('');
                showAgentError('No response from background');
                return;
              }
              if (!res.success) {
                showAgentResponse('');
                const detail =
                  res.data && typeof res.data === 'object'
                    ? (res.data.detail || (res.data.error && res.data.error.detail))
                    : '';
                if (detail) {
                  showAgentError(`${detail}`);
                } else {
                  showAgentError(res.error || 'Request failed');
                }
                if (res.data) {
                  showAgentSteps([{ error: res.data }]);
                }
                return;
              }
              ...
              showAgentError('');
              showAgentResponse(answer);
              showAgentSteps(data.steps || []);

              if (data.session_id) {
                chrome.storage.local.set({ mcpAgentSessionId: data.session_id });
              }
            }
          );
        });
      ...
      if (agentSendBtn) {
        agentSendBtn.onclick = () => sendAgentPrompt();
      }
      if (agentClearBtn && agentPromptInput) {
        agentClearBtn.onclick = () => {
